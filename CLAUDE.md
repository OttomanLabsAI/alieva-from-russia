...
| --- | --- | --- |
| v1.0 | Nastya's Russian lessons find a permanent home | The landing page for Russian with Nastya is now a live site of its own, with every course, club and lesson laid out and a free first lesson one click away. Visitors who wander off the path land on a friendly page in the same style instead of a blank error. |
| v1.1 | The site now makes its own case | A new tab up top lays out the whole offer — what improved over the original page, how the build works, the two prices, and what ownership includes. The first tab now keeps her current site one click away, so old and new can be compared side by side. |
| v1.2 | Three clean tabs and a vocabulary app | The crowded menu is gone: the whole site now sits behind one Website tab, the offer behind another, and between them a full Russian vocabulary builder — a searchable 45,698-word dictionary with a personal word file — now lives on the site itself. |
| v1.3 | Back to the full menu, minus the crumple | The site returns to its previous shape — every section back in the menu alongside the original-site tab, the vocabulary builder set aside for now. The menu also folds into its compact form a little earlier, so it can no longer wrap onto two lines on mid-sized windows. |
